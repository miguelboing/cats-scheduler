...
struct fixed_rate_packet_t
{
    ...
    fixed_rate_packet_t(unsigned int period, unsigned int comp_cost, unsigned int success_rate_req, unsigned int id, unsigned int fixed_rate, unsigned int phase) :
        fixed_rate(fixed_rate), phase(phase)
    {
        count = 0U;
...
{
public:
    FixedRate_buffer(std::vector<fixed_rate_packet_t> packets);
